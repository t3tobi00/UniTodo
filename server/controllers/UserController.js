...
const postLogin = async(req, res, next)=>{
    const invalid_creadentials_message = 'Invalid credentials';
    try{
        const userFromDb = await selectUserByEmail(req.body.email);
        if(userFromDb.rowCount === 0) return next(new ApiError(invalid_creadentials_message))

        const user = userFromDb.rows[0];
        console.log('user', user)
        if(!await compare(req.body.password, user.password)) return next(new ApiError(invalid_creadentials_message, 401))
